"""Common base functions for kageClaw CLI commands."""
...
def _load_runtime_config(config: Optional[str] = None, workspace: Optional[str] = None) -> Config:
    ...
    from kageclaw.config.loader import load_config, set_config_path
...
def _make_provider(config: Config, exit_on_error: bool = True):
    ...
    from kageclaw.thinkers.azure_openai_provider import AzureOpenAIThinker
    from kageclaw.thinkers.base import GenerationSettings
    from kageclaw.thinkers.openai_codex_provider import OpenAICodexThinker
    from kageclaw.thinkers.registry import PROVIDERS, find_by_name
    ...
    if provider_name == "openai_codex" or model.startswith("openai-codex/"):
        ...
        from kageclaw.thinkers.custom_provider import CustomThinker
        ...
        if not current_ready:
            ...
            if not any_ready:
                if exit_on_error:
                    safe_print("\n🐾 [bold]Please run: kageclaw onboard[/bold]")
                    sys.exit(0)
                return None
            else:
                safe_print(
        ...
        if spec and spec.name == "anthropic":
            from kageclaw.thinkers.anthropic_provider import AnthropicThinker
