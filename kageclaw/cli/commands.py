...
app = typer.Typer(
    ...
    help=f"{__logo__} kageclaw - Personal AI Assistant",
...
def version_callback(value: bool):
    if value:
        safe_print(f"{__logo__} kageclaw v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """kageclaw - Personal AI Assistant."""
    pass
...
def onboard(
    ...
    """Initialize kageclaw configuration and workspace."""
...
def web(
    ...
    """Start the kageClaw WebUI in the browser."""
    import os
    import socket
    import subprocess
    import sys
    ...
    from kageclaw.helpers.system import find_free_tcp_port, is_tcp_port_available
    from kageclaw.webui.server import get_auth_token, run_server
    ...
    if with_gateway:
        ...
        os.environ["kageCLAW_WEBUI_URL"] = f"http://127.0.0.1:{port}"
    ...
    safe_print(f"{__logo__} [bold gold1]kageClaw WebUI[/bold gold1]")
...
def desktop(
    ...
    no_auth: bool = typer.Option(
        False,
        "--no-auth",
        help="Disable WebUI auth for this desktop launch. On local Windows source runs auth is already disabled by default unless kageCLAW_AUTH is set.",
    ),
):
    """Start kageClaw in a native desktop window (Windows)."""
    from kageclaw.desktop.launcher import run as launcher_run
...
@app.command()
def status():
    """Show kageclaw status."""
    from kageclaw.config.loader import get_config_path, load_config
    ...
    safe_print(f"{__logo__} [bold]kageclaw Status[/bold]\n")
    safe_print(
...
def channels_status():
    """Show channel status."""
    from kageclaw.config.loader import load_config
    from kageclaw.integrations.registry import discover_all, discover_channel_names
