...
console = Console(
    force_terminal=True if os.environ.get("kageCLAW_FORCE_TERMINAL") else None,
)
...
def safe_print(message: str, **kwargs) -> None:
    ...
    if not _supports_unicode:
        # Simple regex-free replacement for common kageClaw emojis
...
class ThinkingSpinner:
    ...
    def __init__(self, enabled: bool):
        self._spinner = (
            console.status(
                "[dim]kageclaw is [bold gold1]hunting[/bold gold1] for answers...[/dim]",
                spinner="dots",
            )
            if enabled
            else None
...
def print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[gold1]{__logo__} kageclaw[/gold1]")
    console.print(body)
    console.print()
