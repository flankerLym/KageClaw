"""Onboarding and configuration management for the kageClaw CLI."""
...
def _pick_provider(config, env_found: dict[str, str], oauth_found: list[str]):
    ...
    for i, (_, label, env_key, _, is_local, is_oauth) in enumerate(choices, 1):
        if is_local:
            ...
            note = "[dim](OAuth — run: kageclaw provider login)[/dim]"
        elif env_key:
...
def _ask_channel() -> tuple[str, dict[str, Any]] | None:
    ...
    try:
        ...
        mod = importlib.import_module(f"kageclaw.integrations.{chosen}")
        from kageclaw.integrations.registry import discover_all as _da
...
def _show_summary(config_path: Path, provider: str, model: str) -> None:
    ...
    console.print('  Chat:     [cyan]kageclaw agent -m "Hello!"[/cyan]')
    console.print("  WebUI:    [cyan]kageclaw web[/cyan]")
    console.print("  Gateway:  [cyan]kageclaw gateway[/cyan]")
    console.print(
...
def onboard_command(
    ...
    from kageclaw.config.loader import get_config_path, load_config, save_config, set_config_path
    from kageclaw.config.paths import get_workspace_path
    from kageclaw.config.schema import Config
    from kageclaw.helpers.helpers import sync_workspace_templates
    ...
    console.print(
        Panel(
            f"[bold gold1]{__logo__} kageclaw v{__version__}[/bold gold1]\n"
    ...
    if not has_any_provider or not chosen_model:
        ...
        if result:
            ...
            if is_oauth:
                console.print(
                    f"\n  [yellow]Run [bold]kageclaw provider login {pname.replace('_', '-')}[/bold]"
