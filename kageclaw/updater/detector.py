...
PYPI_PACKAGE = "kageclaw"
DOCKER_IMAGE = "rikyz90/kageclaw"
GITHUB_REPO = "RikyZ90/kageClaw"
GITHUB_REPO_URL = f"https://github.com/{GITHUB_REPO}"
UPDATE_MANIFEST_ASSET = "update_manifest.json"
WINDOWS_RELEASE_ASSET = "kageClaw-windows.zip"
...
def get_runtime_root_path() -> Path:
    """Return the root that contains the active kageClaw runtime."""
    return get_runtime_root().resolve()


def get_current_version() -> str:
    """Return the current kageClaw version string."""
    return __version__
...
def _looks_like_source_checkout(root: Path) -> bool:
    return (
        ...
        and (root / "kageclaw" / "__init__.py").exists()
    )
...
def get_installation_method() -> InstallMethod:
    """Detect how kageClaw is currently installed.
