"""Check remote sources for a newer kageClaw update."""
...
from kageclaw.helpers.system import InstallMethod
from kageclaw.updater.detector import (
    DOCKER_IMAGE,
    GITHUB_REPO,
    GITHUB_REPO_URL,
    PYPI_PACKAGE,
...
from kageclaw.updater.manifest import fetch_manifest
...
_DOCKER_TAGS_URL = "https://hub.docker.com/v2/namespaces/rikyz90/repositories/kageclaw/tags?page_size=100"
...
_DEFAULT_HEADERS = {
    ...
    "User-Agent": f"kageClaw/{get_current_version()}",
}
...
def _blank_result(install_method: InstallMethod, current: str) -> dict[str, Any]:
    action_defaults: dict[InstallMethod, dict[str, Any]] = {
        ...
        "docker": {
            ...
            "notes": [
                "If you run kageClaw with docker compose, recreate the service after pulling the new image.",
            ],
        },
        "exe": {
            "action_kind": "automatic",
            ...
            "notes": [
                "kageClaw will automatically download and replace the executable. A UAC prompt may appear.",
            ],
        },
        "source": {
            "action_kind": "manual-command",
...
def _build_notification(result: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        "category": "update",
        "title": "kageClaw update available",
        "body": result.get("summary") or "A newer kageClaw update is available.",
