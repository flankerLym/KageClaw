...
def _auth_enabled() -> bool:
    return os.environ.get("kageCLAW_AUTH", "true").lower() not in ("false", "0", "no", "off")


def _load_or_generate_token() -> str:
    env_token = os.environ.get("kageCLAW_AUTH_TOKEN", "").strip()
    if env_token:
        return env_token
...
def get_cors_origins(port: int = 3000, host: str = "127.0.0.1") -> list[str]:
    env = os.environ.get("kageCLAW_CORS_ORIGINS", "").strip()
    if env == "*":
        return ["*"]
    if env:
