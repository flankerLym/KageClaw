...
OPENROUTER_CALLBACK_BASE_URL_ENV = "kageCLAW_OPENROUTER_CALLBACK_BASE_URL"
...
def _openrouter_headers(extra: dict[str, str] | None = None) -> dict[str, str]:
    headers = {
        "X-Title": "kageClaw",
        "HTTP-Referer": "https://github.com/flankerLym/KageClaw",
    }
    if extra:
...
async def finish_openrouter_oauth(request: Request, jobs: dict):
    ...
    try:
        ...
        asyncio.get_event_loop().call_later(300, lambda: jobs.pop(job_id, None))
        return HTMLResponse(
            _oauth_result_page(True, "OpenRouter was connected successfully. You can close this window and return to kageClaw."),
            status_code=200,
        )
    ...
    for _ in range(max_attempts):
        ...
        try:
            ...
            if access_token:
                ...
                token_dir = os.path.join(home, ".kageclaw", "github_copilot")
                ...
                try:
                    ...
                    if agent_manager.config and agent_manager.config.gateway:
                        ...
                        gateway_hostname = os.environ.get(
                            "kageCLAW_GATEWAY_HOST", "kageclaw-gateway"
                        )
    ...
    async def _run_flow():
        try:
            ...
            if token and getattr(token, "access", None):
                home = os.path.expanduser("~")
                cred_dir = os.path.join(home, ".config", "kageclaw", "openai_codex")
