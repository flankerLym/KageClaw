...
window.selectSession = function (sessionId, el) {
    console.debug('[kage] selectSession clicked:', sessionId);
    ...
    try {
        ...
        if (info) {
            ...
            if (!s) {
                ...
                s.style.cssText = 'display:inline-block;width:14px;height:14px;border:2px solid rgba(0,0,0,0.1);border-left-color:var(--accent);border-radius:50%;margin-left:8px;animation:kage-spin 0.9s linear infinite;vertical-align:middle';
                const nameEl = info.querySelector('.session-name');
                if (nameEl) nameEl.appendChild(s);
            }
        }
    } catch (e) { console.debug('[kage] spinner add failed', e); }
    ...
    (async () => {
        ...
        } catch (e) { console.debug('[kage] loadSession failed', e); }
        ...
        console.debug('[kage] selectSession complete:', sessionId);
    })();
};

// CSS for spinner
const style = document.createElement('style');
style.textContent = '@keyframes kage-spin { from { transform: rotate(0deg);} to { transform: rotate(360deg);} } .history-item.loading { opacity: 0.8; } .history-item.loading .session-name { opacity: 0.9; }';
