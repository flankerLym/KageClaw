        ...
        for (const job of filteredJobs) {
            ...
            if (delBtn) {
                delBtn.addEventListener('click', async () => {
                    const ok = await kageDialog('confirm', 'Delete job', `Delete "${job.name || job.id}"?`, { confirmText: 'Delete', danger: true });
                    if (!ok) return;
                    try {
...
window.clearCompletedJobs = async function () {
    ...
    const ok = await kageDialog('confirm', 'Clear Completed', `Delete ${completedJobs.length} completed or disabled jobs?`, { confirmText: 'Clear All', danger: true });
    if (!ok) return;
...
window.saveJobForm = async function () {
    ...
    try {
        ...
        if (schedKind === 'every') {
            ...
            if (!expr) {
                kageDialog('alert', 'Validation', 'Cron expression is required.', { danger: true });
                return;
            }
            ...
            if (!dtVal) {
                kageDialog('alert', 'Validation', 'Date/time is required for one-shot jobs.', { danger: true });
                return;
            }
            const atMs = new Date(dtVal).getTime();
            if (isNaN(atMs)) {
                kageDialog('alert', 'Validation', 'Invalid date/time.', { danger: true });
                return;
            }
            schedule = { kind: 'at', atMs };
        } else {
            kageDialog('alert', 'Validation', 'Invalid schedule type.', { danger: true });
            return;
        }
        ...
        if (typeof kageDialog === 'function') {
            kageDialog('alert', 'Error', 'Failed to save job: ' + e, { danger: true });
        } else {
