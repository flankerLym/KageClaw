...
async def api_update_manifest(request: Request):
    ...
    try:
        from kageclaw.updater.manifest import fetch_manifest, personal_files_in_manifest
...
async def api_update_apply(request: Request):
    """Apply a kageClaw update or return manual-action guidance."""
    try:
    ...
    try:
        ...
        from kageclaw.webui.ws_handler import _ws_clients
...
async def api_restart_server(request: Request):
    """Restart the kageClaw WebUI server process."""
    async def _do_restart():
