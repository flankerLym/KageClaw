...
from kageclaw.integrations.registry import discover_plugins
...
async def api_list_plugins(request: Request) -> JSONResponse:
    ...
    if "whatsapp" not in installed_channels:
        available.append({
            "name": "kageclaw-channel-whatsapp",
...
async def api_install_plugin(request: Request) -> JSONResponse:
    ...
    if is_running_as_exe():
        return JSONResponse({
            "ok": False,
            "error": "Plugin installation is not supported in the packaged .exe version. Please run kageClaw from a Python environment (pip/source) to install plugins."
        }, status_code=400)
    ...
    if not package.startswith("kageclaw-"):
        return JSONResponse({"error": "Only kageclaw official plugins can be installed"}, status_code=400)
    ...
    if local_path.is_dir():
        ...
        install_target = f"git+https://github.com/flankerLym/KageClaw.git@{tag}#subdirectory={package}"
    ...
    try:
        ...
        from kageclaw.webui.routers.system import (
            _restart_callback,
...
async def api_uninstall_plugin(request: Request) -> JSONResponse:
    ...
    if is_running_as_exe():
        return JSONResponse({
            "ok": False,
            "error": "Plugin uninstallation is not supported in the packaged .exe version. Please run kageClaw from a Python environment (pip/source) to uninstall plugins."
        }, status_code=400)
    ...
    if not package.startswith("kageclaw-"):
        return JSONResponse({"error": "Only kageclaw official plugins can be uninstalled"}, status_code=400)
