"""OS abstraction layer for kageClaw.
...
def is_running_as_exe() -> bool:
    ...
    Checks for PyInstaller (sys.frozen) or a custom launcher named kageClaw.exe.
    """
    if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
        return True
    try:
        if os.path.basename(sys.executable).lower() == "kageclaw.exe":
            return True
    except Exception:
        pass
    return False
...
def get_installation_method() -> InstallMethod:
    """Detect how kageClaw was installed / launched.
