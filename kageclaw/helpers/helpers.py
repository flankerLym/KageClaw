"""Helper functions for the kageClaw ecosystem."""
...
from kageclaw.cli.utils import safe_print
...
def sync_profiles(workspace: Path) -> list[str]:
    ...
    try:
        tpl = pkg_files("kageclaw") / "templates" / "profiles"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("kageclaw") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
