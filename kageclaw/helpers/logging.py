"""Custom logging configuration for kageClaw."""
...
def _is_debug_env() -> bool:
    return os.environ.get("kageCLAW_DEBUG", "").lower() in ("1", "true", "yes", "on")


def setup_kage_logging(level: str = "INFO", show_path: bool = False):
    """
    ...
    kage_icon = "🐾" if supports_unicode else ">>"
    sep_icon = "»" if supports_unicode else ">"

    fmt = (
        "<blue>{time:HH:mm:ss}</blue> "
        "<level>{level: ^8}</level> "
        f"<bold><white>{kage_icon} {{extra[component]: <7}}</white></bold> "
