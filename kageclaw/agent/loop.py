"""Agent loop: the core engine where the kage hunts for answers."""
...
from kageclaw.agent.memory import PackMemory, ScentKeeper
from kageclaw.agent.skills import BUILTIN_SKILLS_DIR
from kageclaw.agent.subagent import SubagentManager
from kageclaw.agent.tools.automation import AutomationTool
from kageclaw.agent.tools.filesystem import EditFileTool, ListDirTool, ReadFileTool, WriteFileTool
from kageclaw.agent.tools.memory_search import MemorySearchTool
from kageclaw.agent.tools.message import MessageTool
from kageclaw.agent.tools.registry import SkillVault
from kageclaw.agent.tools.shell import ExecTool
from kageclaw.agent.tools.spawn import SpawnTool
from kageclaw.agent.tools.web import WebFetchTool, WebSearchTool
...
if TYPE_CHECKING:
    from kageclaw.config.schema import ExecToolConfig, WebSearchConfig
...
class kageBrain:
    ...
    def __init__(
        ...
        self.tool_timeout = config.agents.defaults.tool_timeout if config else int(os.getenv("kageCLAW_TOOL_TIMEOUT", "660"))
        self.loop_wall_timeout = config.agents.defaults.loop_wall_timeout if config else int(os.getenv("kageCLAW_LOOP_WALL_TIMEOUT", "600"))
        subagent_timeout = config.agents.defaults.subagent_timeout if config else int(os.getenv("kageCLAW_SUBAGENT_TIMEOUT", "600"))
    ...
    async def reconfigure(self, new_cfg: Any, new_provider: Any) -> None:
        ...
        logger.info("kageBrain reconfigured (model={})", self.model)
    ...
    async def _connect_mcp(self) -> None:
        ...
        from kageclaw.agent.tools.mcp import connect_mcp_servers
    ...
    def _safe_argv() -> list[str]:
        ...
        if getattr(sys, "frozen", False):
            ...
            for arg in sys.argv[1:]:
                if arg.startswith("-") or arg in kageBrain._ALLOWED_SUBCOMMANDS:
                    safe.append(arg)
            return safe
    ...
    async def close_mcp(self) -> None:
        ...
        if self._mcp_stack:
            ...
            self._mcp_stack = None
        try:
            from kageclaw.agent.tools.mcp import clear_mcp_sessions
            clear_mcp_sessions()
        except Exception:
            pass
    ...
    async def _process_message(
        ...
        if self.provider is None:
            return OutboundMessage(
                channel=msg.channel,
                chat_id=msg.chat_id,
                content="🐕 kage is idle. Please configure an AI provider in the WebUI to start hunting!",
            )
        ...
        if model := session.metadata.get("model"):
            from kageclaw.helpers.model_ids import canonicalize_model_id
