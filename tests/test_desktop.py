...
class TestIsRunningAsExe:
    def test_returns_false_in_normal_python(self):
        from kageclaw.helpers.system import is_running_as_exe
...
class TestGetInstallationMethod:
    def test_exe_when_frozen(self):
        from kageclaw.helpers.system import get_installation_method

        with mock.patch("kageclaw.helpers.system.is_running_as_exe", return_value=True):
            assert get_installation_method() == "exe"

    def test_docker_wins_over_pip(self):
        from kageclaw.helpers.system import get_installation_method

        with mock.patch("kageclaw.helpers.system.is_running_as_exe", return_value=False), \
             mock.patch("kageclaw.helpers.system.is_running_in_docker", return_value=True):
    ...
    def test_pip_when_in_venv(self):
        ...
        with mock.patch("kageclaw.helpers.system.is_running_as_exe", return_value=False), \
             mock.patch("kageclaw.helpers.system.is_running_in_docker", return_value=False), \
             mock.patch("kageclaw.helpers.system.is_running_in_pip_env", return_value=True):
    ...
    def test_source_as_fallback(self):
        ...
        with mock.patch("kageclaw.helpers.system.is_running_as_exe", return_value=False), \
             mock.patch("kageclaw.helpers.system.is_running_in_docker", return_value=False), \
             mock.patch("kageclaw.helpers.system.is_running_in_pip_env", return_value=False):
...
class TestGetAppRoot:
    def test_returns_path_object(self):
        from kageclaw.config.paths import get_app_root

        root = get_app_root()
        assert isinstance(root, Path)

    def test_points_to_kageclaw_dir(self):
        from kageclaw.config.paths import get_app_root

        root = get_app_root()
        assert root.name == ".kageclaw"
    ...
    def test_creates_directory(self, tmp_path):
        ...
        with mock.patch("pathlib.Path.home", return_value=fake_home):
            from kageclaw.config import paths as paths_module
...
class TestAuthTokenPath:
    def test_token_file_under_app_root(self):
        from kageclaw.config.paths import get_app_root
        from kageclaw.webui.auth import AUTH_TOKEN_FILE
...
class TestCacheFilePath:
    def test_cache_file_under_app_root(self):
        from kageclaw.config.paths import get_app_root
        from kageclaw.updater.checker import _CACHE_FILE
...
class TestDesktopConfig:
    def test_defaults(self):
        from kageclaw.config.schema import DesktopConfig
...
class TestWindowsAppUserModelId:
    def test_sets_app_user_model_id_for_source_launch(self):
        ...
        import kageclaw.desktop.launcher as launcher
    ...
    def test_apply_windows_window_icon_sets_class_icons(self):
        ...
        with mock.patch.object(launcher, "_resolve_windows_window_handle", return_value=456), \
            ...
            launcher._apply_windows_window_icon(window, "C:/icons/kageclaw.ico")
...
class TestDesktopRuntime:
    ...
    def test_start_sets_shared_auth_token_env(self):
        ...
        with mock.patch.object(rt, "_load_config"), \
             ...
             mock.patch("kageclaw.webui.auth.get_auth_token", return_value="shared-token"):
            rt.start()

        assert os.environ.get("kageCLAW_AUTH_TOKEN") == "shared-token"
    ...
    def test_resolve_gateway_ports_uses_fallback_when_configured_ports_busy(self):
        ...
        with mock.patch(
            "kageclaw.desktop.runtime.is_tcp_port_available",
            side_effect=[False] * 30,
        ), mock.patch(
            "kageclaw.desktop.runtime.find_free_tcp_port",
...
class TestDesktopLauncherAuth:
    def test_local_windows_source_defaults_auth_off(self):
        from kageclaw.desktop import launcher

        with mock.patch.dict(os.environ, {}, clear=False):
            os.environ.pop("kageCLAW_AUTH", None)
            with mock.patch("kageclaw.desktop.launcher.get_os_type", return_value="windows"), \
                 mock.patch("kageclaw.desktop.launcher.is_running_as_exe", return_value=False):
                launcher._configure_desktop_auth()

            assert os.environ.get("kageCLAW_AUTH") == "false"

    def test_explicit_env_override_is_preserved(self):
        from kageclaw.desktop import launcher

        with mock.patch.dict(os.environ, {"kageCLAW_AUTH": "true"}, clear=False):
            with mock.patch("kageclaw.desktop.launcher.get_os_type", return_value="windows"), \
                 mock.patch("kageclaw.desktop.launcher.is_running_as_exe", return_value=False):
                launcher._configure_desktop_auth(disable_auth=True)

            assert os.environ.get("kageCLAW_AUTH") == "true"
    ...
    def test_frozen_build_does_not_disable_auth_implicitly(self):
        ...
        with mock.patch.dict(os.environ, {}, clear=False):
            os.environ.pop("kageCLAW_AUTH", None)
            with mock.patch("kageclaw.desktop.launcher.get_os_type", return_value="windows"), \
                 mock.patch("kageclaw.desktop.launcher.is_running_as_exe", return_value=True):
                launcher._configure_desktop_auth()

            assert "kageCLAW_AUTH" not in os.environ
    ...
    def test_resolve_window_config_uses_runtime_config(self):
        ...
        from kageclaw.desktop.window_state import WindowState

        with mock.patch("kageclaw.desktop.launcher.load_window_state") as mock_load:
    ...
    def test_desktop_debug_requires_explicit_env(self):
        ...
        with mock.patch.dict(os.environ, {}, clear=False):
            os.environ.pop("kageCLAW_DESKTOP_DEBUG", None)
            assert launcher._desktop_debug_enabled() is False

        with mock.patch.dict(os.environ, {"kageCLAW_DESKTOP_DEBUG": "true"}, clear=False):
            assert launcher._desktop_debug_enabled() is True

    def test_get_icon_path_uses_assets_dir(self, tmp_path):
        from kageclaw.desktop import launcher

        icon_path = tmp_path / "kageclaw.ico"
        icon_path.write_bytes(b"ico")

        with mock.patch("kageclaw.desktop.launcher.get_assets_dir", return_value=tmp_path):
            assert launcher._get_icon_path() == str(icon_path)


class TestDesktopMainEntrypoint:
    def test_main_imports_and_runs_launcher(self):
        from kageclaw.desktop.__main__ import main

        fake_launcher = types.ModuleType("kageclaw.desktop.launcher")
        fake_launcher.run = mock.Mock()

        with mock.patch.dict(sys.modules, {"kageclaw.desktop.launcher": fake_launcher}):
            with mock.patch("kageclaw.desktop.__main__.setup_kage_logging"):
                main()
    ...
    def test_main_shows_visible_error_on_failed_startup(self):
        ...
        with mock.patch.dict(sys.modules, {"kageclaw.desktop.launcher": fake_launcher}):
            with mock.patch("kageclaw.desktop.__main__.setup_kage_logging"), mock.patch(
                "kageclaw.desktop.__main__._show_startup_error"
            ) as mock_show_error:
