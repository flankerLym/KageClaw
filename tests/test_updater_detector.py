...
def _make_source_tree(root: Path) -> None:
    (root / "KAGECLAW").mkdir(parents=True, exist_ok=True)
    (root / "pyproject.toml").write_text('[project]\nname = "KAGECLAW"\n', encoding="utf-8")
    (root / "README.md").write_text("# KAGECLAW\n", encoding="utf-8")
    (root / "KAGECLAW" / "__init__.py").write_text("__version__ = '0.0.0'\n", encoding="utf-8")


def test_detector_prefers_source_checkout_over_venv(tmp_path, monkeypatch):
    from KAGECLAW.updater import detector
...
def test_detector_accepts_official_remote_urls(monkeypatch):
    ...
    monkeypatch.setattr(
        detector,
        "get_git_remote_url",
        lambda root=None: "git@github.com:RikyZ90/KAGECLAW.git",
    )
